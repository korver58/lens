...
def make_map_inv(map_src, scale):
    h, w = map_src[0].shape
    ...
    map_inv = np.zeros((h//scale, w//scale, 2), np.float32)
    for y in range(h):
        for x in range(w):
            fx, ix = math.modf(map[y][x][0])
            fy, iy = math.modf(map[y][x][1])
            if ix < w//scale and iy < h//scale:
...
def test():
    ...
    scale = 2
    new_cam_mat, roi_size = cv2.getOptimalNewCameraMatrix(cam_mat, dist_coef, (w, h), 1, (w*scale, h*scale))
    map_undis = cv2.initUndistortRectifyMap(cam_mat, dist_coef, None, new_cam_mat, (w*scale, h*scale), cv2.CV_32FC1)
    undist = cv2.remap(src, map_undis[0], map_undis[1], cv2.INTER_CUBIC)
    cv2.imshow("undis", undist)

    map_inv = make_map_inv(map_undis, scale)
    redist = cv2.remap(undist, map_inv[0], map_inv[1], cv2.INTER_CUBIC)
    cv2.imshow("redist", redist)
    cv2.imwrite("out.png", redist)
